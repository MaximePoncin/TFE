...
class CreateAdmin extends Component {
  ...
  handleSubmit(e) {
    if(this.validateForm(e)) {
      ...
      UserModel.create(JSON.stringify(admin))
      .then(receivedUser => {
  ...
  validateForm(e) {
    ...
    if(!validator.isAlphanumeric(this.state.adminStreetNum.replace(/ /, ''))){
      this.setState({
        adminStreetNumValidation: {
          status: "error",
    ...
    if(!validator.isAlphanumeric(this.state.adminPassword)){
      this.setState({
        adminPasswordValidation: {
          status: "error",
          text: Locale[localStorage.getItem('LPC_beerTour_locale')].validation.mustBeAlphaNum
    ...
    if(this.state.adminPassword !== this.state.adminPasswordConf){
      this.setState({
        adminPasswordConfValidation: {
          status: "error",
    ...
    return(
      <Grid>
        <Row className="show-grid">
          <Col xs={12}>
            <h1>{Locale[localStorage.getItem('LPC_beerTour_locale')].admin.createAdmin}</h1>
          </Col>
        </Row>
            ...
            <fieldset>
              ...
              <Row className="show-grid">
                  <Col xs={10}>
                    <FormGroup
                      bsSize="sm"
                      validationState={this.state.adminPasswordValidation.status}
                    >
                      <ControlLabel>{Locale[localStorage.getItem('LPC_beerTour_locale')].admin.password}</ControlLabel>
                      <FormControl
                        name="adminPassword"
                        type="password"
                        value={this.state.adminPassword}
                        placeholder={Locale[localStorage.getItem('LPC_beerTour_locale')].admin.passwordPlaceholder}
                        onChange={this.handleUserInput}
                        required
                      />
                      <HelpBlock>
                        {this.state.adminPasswordValidation.text}
                      </HelpBlock>
                    </FormGroup>
                  </Col>
              </Row>
              ...
              <Row className="show-grid">
                  <Col xs={10}>
                    <FormGroup
                      bsSize="sm"
                      validationState={this.state.adminPasswordConfValidation.status}
                    >
                      <ControlLabel>{Locale[localStorage.getItem('LPC_beerTour_locale')].admin.passwordConf}</ControlLabel>
                      <FormControl
                        name="adminPasswordConf"
                        type="password"
                        value={this.state.adminPasswordConf}
                        placeholder={Locale[localStorage.getItem('LPC_beerTour_locale')].admin.passwordConfPlaceholder}
                        onChange={this.handleUserInput}
                        required
                      />
                      <HelpBlock>
                        {this.state.adminPasswordConfValidation.text}
                      </HelpBlock>
                    </FormGroup>
                  </Col>
              </Row>
          ...
          <fieldset>
            ...
            <Row className="show-grid">
                <Col xs={10}>
                  ...
                  >
                    <ControlLabel>{[localStorage.getItem('LPC_beerTour_locale')].admin.city}</ControlLabel>
                    <FormControl
                      name="adminCity"
                      type="text"
