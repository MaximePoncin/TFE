...
var server = new Hapi.server(require('./config/APIConnector.server.config'));
...
const init = async () => {
  ...
  try {
    await server.start();
    console.log('APIconnector service running at: ', server.info.uri);
  } catch (err) {
    console.log(err);
  }

  // server.route(require('./routes'));
  // // server.route(require('./routes/CampaignRoutes'));
};
