...
class EditUser extends Component {
  ...
  componentWillMount() {
    UserModel.get(this.props.concernedUserId)
    .then(receivedUser => {
      console.log(receivedUser);
      this.setState({
        user: receivedUser,
        userSurname: receivedUser.person.surname,
        userGivenName: receivedUser.person.givenName,
        userBirthdate: moment(receivedUser.person.birthDate).toDate(),
        userStreet: receivedUser.address.street,
        userStreetNum: receivedUser.address.num,
        userPostalCode: receivedUser.address.postalCode,
        userCity: receivedUser.address.city,
        userCountry: receivedUser.address.country,
        userMail: receivedUser.mail,
        userPhone: receivedUser.phoneNumber,
        userPassword: receivedUser.password
      })
    })
    .catch(err => {
      console.log(err);
  ...
  handleSubmit(e) {
    if(this.validateForm(e)) {
      ...
      UserModel.update(this.props.concernedUserId, JSON.stringify(user))
      .then(receivedUser => {
        console.log(receivedUser);
        console.log("success");
        this.goToUsers();
    ...
    return(
      <Grid>
        <Row className="show-grid">
          <Col xs={12}>
            <h1>{Locale[localStorage.getItem('LPC_beerTour_locale')].userProfil.edit}</h1>
          </Col>
        </Row>
